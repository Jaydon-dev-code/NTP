...
{
    /// <summary>
    /// 六分厂6-3装配 AB托盘绑定 专用读取线程
    /// 独立线程持续读取 A托盘绑定 / B托盘绑定 点位，与A/B线采集线程并行：
    /// 把 b 线托盘号 更新到 LineA 数据库，供B线采集时查询
    /// </summary>
    public class Factory6Workshop6_3AssemblyLineABBinding : DataCollectionServiceAbstract
    {
        private const string _lineName = "B线";
        private const string _aBindPointName = "A托盘绑定";
        private const string _bBindPointName = "B托盘绑定";

        protected override string _serviceName => "六分厂6-3AB托盘绑定";

        private readonly Tb_EquipmentRepository _equipmentRepository;
        private readonly Tb_LineARepository _aLineRepository;
        ...
        public Factory6Workshop6_3AssemblyLineABBinding(
            Tb_EquipmentRepository equipmentRepository,
            Tb_LineARepository aLineRepository,
        ...
        protected override async Task<Result<object>> InteractAsync()
        {
            ...
            if (aResult.IsSuccess && bResult.IsSuccess)
            {
                ...
                // 条件：a b 托盘号相同 且 a 托盘号非0/空 且 与上次数据不同
