...
namespace SL.MLineDataPrecisionTracking.Infrastructure.PLCCommunication
{
    public class S7Communication : IPlcCommunication, IDisposable
    {
        ...
        public Result<DevPlcPointDto> Read(DevPlcPointDto readPlcInfo)
        {
            try
            {
                int db = int.Parse(readPlcInfo.Prefix);
                ...
                else
                {
                    ...
                    if (raw == null)
                        ...
                        readPlcInfo.Value = ConvertBytes(raw, readPlcInfo.DataType, readPlcInfo.Length);
                }
                return Result<DevPlcPointDto>.Success(readPlcInfo);
            }
            catch (Exception ex)
            {
                Log.Warning("[S7]读取异常：{Ip}-{Port}-DB{Db}.{Addr} {Ex}",
        ...
        public Result<List<DevPlcPointDto>> Read(List<DevPlcPointDto> readPlcInfo)
        {
            try
            {
                var items = readPlcInfo.Select(x =>
                {
                    ...
                    return new { Dto = x, Db = db, ByteOff = byteOff };
                }).ToList();

                foreach (var group in items.GroupBy(x => new { x.Dto.IpAddress, x.Dto.Port, x.Db }))
                {
                    var sorted = group.OrderBy(x => x.ByteOff).ToList();
                    int startAddr = sorted.Min(x => x.ByteOff);
                    var last = sorted.Last(); 
                    int endAddr = last.ByteOff + last.Dto.Length * last.Dto.DataType.GetTypeByteLength(); 
                    ...
                    foreach (var item in sorted)
                    {
                        ...
                        if (raw != null)
                            item.Dto.Value = ConvertBytes(raw, offset, item.Dto.DataType, item.Dto.Length);
                        else
        ...
        public Result Write(DevPlcPointDto devPlcPointDto)
        {
            try
            {
                ...
                {
                    bool val = devPlcPointDto.Value != null && devPlcPointDto.Value[0].ToString() == "True";
                    if (bitOffset > 0)
            ...
            {
                Log.Warning("[S7]写入异常：{Ip}-{Port}-DB{Db}.{Addr} {Ex}",
        ...
        private static List<object> ConvertBytes(byte[] buffer, int start, TypeCode type, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                switch (type)
                {
                    ...
                    case TypeCode.Byte:
                        ...
                        break;
                    //case TypeCode.Int16:
                    //    result.Add(Conversion.ConvertToShort(buffer, index));
                    //    break;
                    //case TypeCode.UInt16:
                    //    result.Add((ushort)Conversion.ConvertToUshort(buffer, index));
                    //    break;
                    //case TypeCode.Int32:
                    //    result.Add(Conversion.ConvertToInt(buffer, index));
                    //    break;
                    //case TypeCode.UInt32:
                    //    result.Add((uint)Conversion.ConvertToUInt(buffer, index));
                    //    break;
                    //case TypeCode.Single:
                    //    result.Add(Conversion.ConvertToFloat(buffer, index));
                    //    break;
        ...
        private static byte[] ToS7Bytes(object value, TypeCode type)
        {
            switch (type)
            {
                case TypeCode.Byte: return new byte[] { Convert.ToByte(value) };
                case TypeCode.Int16: return BitConverter.GetBytes(Convert.ToInt16(value));
                case TypeCode.UInt16: return BitConverter.GetBytes(Convert.ToUInt16(value));
                case TypeCode.Int32: return BitConverter.GetBytes(Convert.ToInt32(value));
                case TypeCode.UInt32: return BitConverter.GetBytes(Convert.ToUInt32(value));
                case TypeCode.Single: return BitConverter.GetBytes(Convert.ToSingle(value));
                case TypeCode.Double: return BitConverter.GetBytes(Convert.ToDouble(value));
                default: return new byte[] { Convert.ToByte(value) };
            }
        }
